# attack-tester-windows.ps1 - Windows PowerShell Attack Tester for VulnShop
...
Write-Host "⚠️  NON-DESTRUCTIVE ATTACKS ONLY" -ForegroundColor Green
...
function Execute-Attack {
    ...
    try {
        ...
        if ($response.StatusCode -eq 200) {
            Write-Host "⚠️  Attack succeeded: HTTP $($response.StatusCode)" -ForegroundColor Red
        ...
        if ($status -eq 403 -or $status -eq 400) {
            ...
            Write-Host "🔒 Authentication required: HTTP $status" -ForegroundColor Yellow
        } else {
...
do {
    ...
    for ($i = 1; $i -le 20; $i++) {
        ...
        try {
            ...
            if ($_.Exception.Response -and $_.Exception.Response.StatusCode.value__ -eq 429) {
                Write-Host "`nRate limit triggered at request $i!" -ForegroundColor Green
                $rateLimited = $true
                break
            }
        }
    ...
    if (-not $rateLimited) {
        Write-Host "⚠️  No rate limiting detected after 20 requests" -ForegroundColor Yellow
    }
    ...
    if ($Continuous) {
        Write-Host "`n⏳ Waiting 5 seconds before next iteration..." -ForegroundColor Yellow
